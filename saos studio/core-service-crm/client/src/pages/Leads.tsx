...
import { Search, Plus, MapPin, Globe, Star, Loader2, Import } from 'lucide-react';
...
export default function Leads() {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      await api.importGMaps(importQuery);
      setImportQuery('');
      await load();
    } catch (e: any) {
    ...
    <div className="p-8 max-w-7xl space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-neutral-400 text-sm mt-1">Google Maps businesses & manual entries.</p>
        </div>
        <button onClick={() => nav('/leads/new')} className="flex items-center gap-2 bg-saos-600 hover:bg-saos-500 text-black font-semibold px-4 py-2 rounded-lg text-sm transition-colors">
      ...
      <div className="glass rounded-xl p-4 flex flex-col md:flex-row gap-3">
        ...
        <div className="flex gap-2">
          <input value={importQuery} onChange={e => setImportQuery(e.target.value)} placeholder="Google Maps query…"
            className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-saos-500 w-64" />
          <button onClick={handleImport} disabled={importing}
            className="bg-white/10 hover:bg-white/20 disabled:opacity-50 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
